<template>
	<div class="login">
		...
		<form @submit.prevent="authorizeAccount" class="validation-form">
			<span id="inputs">
				<input autofocus class="input-bar" type="text" placeholder="Username" v-model="username" required />
				...
				try {
					if (localStorage.getItem(username) != null) {
						if (await bcrypt.compare(password, localStorage.getItem(username))) {
							...
							sessionStorage.setItem('arcHome', 'false');
							sessionStorage.setItem('arcAccounts', 'false');

							if (arcData.slidesEnabled === 'false') {
								this.$router.push('/main/home');
							} else {
								this.$router.push('/arcSlides');
							}
						} else {
